...
import sbt.internal.util.{ HNil, SingletonCache }
import sbt.internal.util.CacheImplicits._

import scala.json.ast.unsafe.JValue
import sjsonnew.support.scalajson.unsafe.{ CompactPrinter, Converter, Parser }

object CacheIvy extends LibraryManagementCodec with OtherFormats { // IvyFormats with OtherFormats {

  implicit lazy val isoString: IsoString[JValue] = IsoString.iso(CompactPrinter.apply, Parser.parseUnsafe)

  lazy val updateIC = implicitly[SingletonCache[(IvyConfiguration, ModuleSettings, UpdateConfiguration)]]
  lazy val moduleIDSeqIC = implicitly[SingletonCache[Seq[ModuleID]]]
  implicit lazy val classpathFormat = implicitly[JsonFormat[Map[String, Seq[File]]]]
