...
import Predef.{ conforms => _, StringFormat => _, _ }
import java.io.File
...
object Package {
  ...
  import CacheIvy.streamFormat
  implicit def manifestFormat: JsonFormat[Manifest] = streamFormat(_ write _, in => new Manifest(in))
