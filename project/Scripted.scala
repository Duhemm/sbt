...
object Scripted {
  ...
  def doScripted(launcher: File, scriptedSbtClasspath: Seq[Attributed[File]], scriptedSbtInstance: ScalaInstance, sourcePath: File, args: Seq[String], bootProperties: File => String, prescripted: File => Unit): Unit = {
    ...
    val bootPropertiesFileName = "scripted-boot-properties"
    val launcherVmOptions = Array(
      "-XX:MaxPermSize=256M", "-Xmx1G", // increased after a failure in scripted source-dependencies/macro
      s"-Dsbt.boot.properties=$bootPropertiesFileName") // adds the alternative local repository
      ...
      try {
        ...
        val callback = new java.util.AbstractList[File] {
          override def add(x: File): Boolean = {
            IO.write(x / bootPropertiesFileName, bootProperties(ivyHome))
            prescripted(x)
            false
          }
        ...
        bridge.run(sourcePath, false, args.toArray, launcher, launcherVmOptions, callback)
